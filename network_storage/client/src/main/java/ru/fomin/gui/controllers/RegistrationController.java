...
public class RegistrationController {
    ...
    void initialize() {
        ...
        btn_cancel.setOnAction(event -> requestHandler.exitToAuthentication(btn_cancel));

        btn_registration.setOnAction(event -> registration());

        ResponseHandler.setRegistrationController(this);
    }
    ...
    private void registration() {
        ...
        if (!isWaitingResponse && ControllersUtil.validation(login, password, field_repeat_password.getText())) {
            ...
            requestHandler.registration(login, password);
        } else {
            clearFields();
        }
