...
public class FileSendOptimizer {
    ...
    public void sendFile(Path path, Consumer<DataPackage> sendAction) throws IOException {
    ...
    private void sendFull(Path path, Consumer<DataPackage> sendAction) throws IOException {
        DataPackage pack = new FileDataPackage(path);
        sendAction.accept(pack);
    }


    private void sendByChunks(Path path, Consumer<DataPackage> sendAction) throws IOException {
